...
//        System.out.println(makeAuthorJson(a));
     ...
     * 用于拼接最后返回给前端的字符串
     * */
    public static String getAiFinalAnswer(String retrieveAnswer, String visualAnswer){
        String result = "";
    ...
    //AI_FIELD_OPER_AREA BELOW
    public static<T> String getAiAns(ArrayList<T> list, String type){
        Gson gs = new Gson();
    ...
    public static<T> String getHarfAiJson(ArrayList<Paper> paperList, ArrayList<CnNews> cnNewsList, ArrayList<News> newsList, ArrayList<Patent_new> patentList) {
        String ans = getAiAns(paperList,"papers");
        //开始添加patent结果
        ans = ans.substring(0,ans.length()-1); //删去"}"
        ans += ",";
        String pa = getAiAns(patentList,"patents");
        ...
        ans = ans.substring(0,ans.length()-1); //删去"}"
        ans += ",";
        String cnnews = getAiAns(cnNewsList,"CN_News");
        cnnews = cnnews.substring(1);
        ans = ans + cnnews;
        ans = ans.substring(0,ans.length()-1); //开始添加news
        ans += ",";
        String news = getAiAns(newsList,"news");
